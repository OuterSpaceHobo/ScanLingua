...
async function gotMessage(request, sender, sendResponse) {
    ...
    if (request.type == "request-AddAnkiCard") {
        ...
        try {
            ...
            if (request.kanjiCard !== undefined) {
                ...
                const addedCard = await AddKanjiCard(visionText, b64, translation, kanjiData, deckName)
                ...
                const addedCard = await AddAnkiCard(visionText, b64, translation, deckName)
                console.log(`[Anki] sendAnkiResult requestId=${request.requestId} success`)
                sendEvent('anki_card_exported', { anki_export_count: 1 })
                await sendAnkiResult(request.requestId, addedCard)
            }
            ...
            await sendAnkiResult(request.requestId, { error: 'Anki is not running. Launch Anki and try again.' })
        }
    ...
    if (request.type == "request-available-targets") {
        ...
        const available = await getAvailableTargets(request.source, TARGET_LANGUAGE_CANDIDATES);
