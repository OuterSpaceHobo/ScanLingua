...
const App = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const langResult = await new Promise<Record<string, unknown>>(resolve =>
        chrome.storage.local.get(["languageConfig"], resolve))
      ...
      try {
        ...
        store.dispatch(initializeVision("OCR failed: " + String(e)))
      }
      ...
      if (visionOk) {
        ...
        await Promise.allSettled([
          requestTranslation(undefined).then(
            ...
            (e) => { console.error(`[app] requestTranslation FAILED:`, e); store.dispatch(initializeTranslation("Translation failed: " + String(e))) }
          ),
          requestAnnotation(srcLang).then(
            ...
            (e) => {
              store.dispatch(initializeAnnotation("no annotation"))
            ...
            (() => {
              if(loading) {
                      ...
                      return (
                        <BottomDiv>
                          <InfoP>detected text</InfoP>
                          <ContentP>Text not detected. Try to crop a larger text portion.</ContentP>
                        </BottomDiv>
                      )
                  } else {
                      return (
