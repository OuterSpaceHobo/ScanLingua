...
export function CloseButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button
      className="p-0 w-[35px] h-[35px] rounded-[5px] bg-primary border-0 text-black cursor-pointer shadow-card"
      onClick={onClick}>
      {children}
    </button>
  )
...
export function DragButton({ children, onMouseDown }: { children: React.ReactNode; onMouseDown: (e: React.MouseEvent) => void }) {
  return (
    <button
      className="p-0 w-[35px] h-[35px] rounded-[5px] mt-[5px] bg-primary border-0 text-white cursor-grab active:cursor-grabbing shadow-card"
...
export function AnkiButton({ children, onClick, title }: { children: React.ReactNode; onClick: () => void; title?: string }) {
  return (
    <button
      className="p-0 w-[35px] h-[42px] rounded-[5px] mt-[5px] bg-primary border-0 text-black cursor-pointer shadow-card animate-fade-in"
      onClick={onClick}
      title={title}>
      {children}
    </button>
...
export function EditButton({ children, onClick, style }: { children: React.ReactNode; onClick: (e: { preventDefault: () => void }) => void; style?: React.CSSProperties }) {
  return (
    <button
      className="p-0 w-fit h-fit rounded-[5px] bg-transparent border-0 text-black underline decoration-black decoration-solid decoration-1 cursor-pointer align-text-top animate-fade-in
        relative top-[-4px] left-[5px]"
      onClick={onClick}
      style={style}>
      {children}
    </button>
...
export function AudioButton({ children, onClick, disabled, title, style }: {
  ...
  return (
    <button
      className="p-0 w-[23px] h-[23px] rounded-none bg-transparent border-0 text-error cursor-pointer align-text-top"
      onClick={onClick}
      disabled={disabled}
      title={title}
      style={style}>
...
export function TileButton({ children, onClick, dropdown, index }: {
  children: React.ReactNode;
  onClick: () => void;
  ...
  return (
    <button
      className={`p-0 w-[60px] h-[60px] rounded-none bg-transparent border-0 cursor-pointer align-text-top animate-fade-in ${dropdown === index ? 'text-error' : 'text-black'}`}
      onClick={onClick}>
      {children}
    </button>
  )
...
export function Spinner() {
  return (
    <div className="border-2 border-white border-t-primary rounded-full h-[30px] w-[30px] animate-spin" />
  )
}
