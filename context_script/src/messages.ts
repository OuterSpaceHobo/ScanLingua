...
const requests: Map<number, (value: unknown) => void> = new Map()
    ...
    export async function requestAddAnkiCard (kanjiCard: Record<string, string | number | undefined> | undefined, labels: Record<string, string> = {}) {
        const requestId = getRandomInt(100000)
        ...
        const res = await new Promise<unknown>((resolve) => {
            ...
            chrome.runtime.sendMessage({type: "request-AddAnkiCard", requestId, kanjiCard, labels})
        })
        return res
    }
